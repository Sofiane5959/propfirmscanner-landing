...
export type AlertType = 
  | 'drawdown_warning'    // 80% of limit
  | 'drawdown_critical'   // 95% of limit
        ...
        <div style="${baseStyle} padding: 40px; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #111827; border-radius: 12px; padding: 25px; margin: 25px 0;">
            <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
              <span style="color: #9ca3af;">Drawdown actuel</span>
              <span style="color: #f59e0b; font-weight: bold; font-size: 20px;">${data.currentValue.toFixed(2)}$</span>
            </div>
            <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
              <span style="color: #9ca3af;">Limite maximale</span>
              <span style="color: #e5e7eb; font-weight: bold;">${data.limitValue.toFixed(2)}$</span>
            </div>
    ...
    drawdown_critical: {
      ...
      html: `
        <div style="${baseStyle} padding: 40px; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background: #111827; border: 2px solid #ef4444; border-radius: 12px; padding: 25px; margin: 25px 0;">
            ...
            <p style="color: #ef4444; text-align: center; margin-top: 15px;">
              Il vous reste <strong>${(data.limitValue - data.currentValue).toFixed(2)}$</strong> avant la limite
            </p>
          </div>

          <div style="background: #7f1d1d20; border-radius: 8px; padding: 15px; margin: 20px 0;">
            <p style="color: #fca5a5; margin: 0; font-size: 14px;">
              💡 <strong>Conseil:</strong> Envisagez de réduire la taille de vos positions ou de prendre une pause trading.
            </p>
          </div>
    ...
    profit_target_reached: {
      ...
      html: `
        <div style="${baseStyle} padding: 40px; max-width: 600px; margin: 0 auto;">
          ...
          <p style="color: #e5e7eb; font-size: 16px; text-align: center;">
            🚀 Prochaine étape: Vérifiez les conditions pour passer à la phase suivante!
          </p>
    ...
    challenge_expiring: {
      ...
      html: `
        <div style="${baseStyle} padding: 40px; max-width: 600px; margin: 0 auto;">
          ...
          <p style="color: #e5e7eb; font-size: 16px;">
            Votre challenge <strong style="color: #10b981;">${data.accountName}</strong> 
            expire bientôt!
          </p>
          ...
          <div style="background: #1f2937; border-radius: 8px; padding: 20px; margin: 20px 0;">
            <p style="color: #e5e7eb; margin: 0 0 10px 0;"><strong>Progression actuelle:</strong></p>
            <p style="color: #10b981; margin: 0; font-size: 18px;">${data.currentValue.toFixed(2)}$ / ${data.limitValue.toFixed(2)}$ (${data.percentage.toFixed(1)}%)</p>
          </div>
    ...
    daily_loss_warning: {
      ...
      html: `
        <div style="${baseStyle} padding: 40px; max-width: 600px; margin: 0 auto;">
          ...
          <p style="color: #e5e7eb; font-size: 16px;">
            Votre compte <strong style="color: #10b981;">${data.accountName}</strong> 
            approche de la limite de perte journalière.
          </p>
          ...
          <div style="background: #78350f20; border-radius: 8px; padding: 15px; margin: 20px 0;">
            <p style="color: #fcd34d; margin: 0; font-size: 14px;">
              💡 <strong>Conseil:</strong> Considérez d'arrêter le trading pour aujourd'hui.
            </p>
          </div>
