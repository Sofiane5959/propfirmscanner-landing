  ...
  if (status === 'success') {
    return (
      <div className="flex flex-col items-center gap-3 py-4">
        <div className="w-12 h-12 bg-accent-hover rounded-full flex items-center justify-center">
          <CheckCircle className="w-6 h-6 text-white" />
        </div>
        <p className="text-accent font-semibold text-lg">You're on the list!</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
      <input
        ...
        required
        className="flex-1 px-4 py-3 bg-bg-elevated border border-border rounded-xl text-white placeholder-text-muted focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent"
        disabled={status === 'loading'}
      />
      <button
        type="submit"
        disabled={status === 'loading' || !email.trim()}
        className="px-6 py-3 bg-gradient-to-r from-accent to-accent hover:from-accent hover:to-accent text-white font-semibold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 whitespace-nowrap"
      >
        {status === 'loading' ? (
  ...
  return (
    <section className="relative mt-20 mb-20">
      ...
      <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="flex items-center justify-center gap-4 mb-8">
            <div className="text-center">
              <div className="flex items-baseline gap-2 justify-center">
                ...
                <span className="text-text-muted line-through text-xl">$499.99</span>
              </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mb-12">
          {modules.map((mod, i) => {
            ...
            return (
              <div
                key={i}
                className="relative bg-bg-elevated/60 border border-border rounded-2xl p-6 hover:border-yellow-500/30 transition-colors group"
              >
                {/* Lock overlay */}
                <div className="absolute top-4 right-4">
                  <Lock className="w-4 h-4 text-text-muted group-hover:text-yellow-500/50 transition-colors" />
                </div>
        ...
        {/* Waitlist CTA box */}
        <div className="bg-gradient-to-br from-bg-elevated to-dark-700 border border-yellow-500/20 rounded-2xl p-8 md:p-10 text-center">
