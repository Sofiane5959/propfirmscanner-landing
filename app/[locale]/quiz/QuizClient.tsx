...
const COLOR_MAP: Record<string, { bg: string; border: string; text: string; icon: string; glow: string }> = {
  emerald: { bg: 'bg-accent/10', border: 'border-accent/60', text: 'text-accent', icon: 'text-accent', glow: 'shadow-emerald-500/20' },
...
const ProgressBar = ({ step, total }: { step: number; total: number }) => (
  <div className="flex items-center gap-3 mb-8">
    ...
    <span className="text-xs text-text-muted font-medium tabular-nums">{step}/{total}</span>
  </div>
)

const MatchBar = ({ percent }: { percent: number }) => {
  const color = percent >= 85 ? 'from-accent to-accent'
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-xs font-bold tabular-nums ${percent >= 85 ? 'text-accent' : percent >= 70 ? 'text-blue-400' : 'text-text-secondary'}`}>
        {percent}%
      </span>
    </div>
  )
...
export default function QuizClient() {
    ...
    return (
      <div className="min-h-screen bg-bg-base flex items-center justify-center px-4 py-20">
        <div className="w-full max-w-lg text-center">
          ...
          <button
            onClick={() => setStep(1)}
            className="inline-flex items-center gap-2 px-8 py-4 bg-accent-hover hover:brightness-110 text-white font-bold text-base rounded-xl transition-all shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 hover:-translate-y-0.5"
          >
            Start matching
            <ChevronRight className="w-5 h-5" />
          </button>

          <p className="text-xs text-text-muted mt-4">No account required</p>
        </div>
      </div>
    )
  }
  ...
  if (isComplete) {
    ...
    return (
      <div className="min-h-screen bg-bg-base px-4 py-20">
        <div className="max-w-2xl mx-auto">
          {/* Header */}
          <div className="text-center mb-10">
            <p className="text-accent text-sm font-medium mb-2">Your personalized results</p>
            <h2 className="text-3xl font-bold text-white mb-2">Here are your top matches</h2>
            <p className="text-text-muted text-sm">Based on your experience, style, budget and priorities.</p>
          </div>

          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map(i => (
                <div key={i} className="bg-bg-elevated border border-border rounded-2xl p-5 animate-pulse h-36" />
              ))}
            </div>
          ) : (
                ...
                return (
                  <div
                    ...
                    className={`bg-bg-elevated border rounded-2xl p-5 transition-all ${
                      i === 0 ? 'border-accent/40 shadow-lg shadow-emerald-500/10' : 'border-border'
                    }`}
                  >
                    {/* Badge */}
                    ...
                    <div className="flex items-start gap-4">
                      <div className="w-14 h-14 rounded-xl bg-white flex items-center justify-center overflow-hidden border border-gray-200 p-1.5 flex-shrink-0">
                        {firm.logo_url
                          ...
                          : <span className="text-xl font-bold text-accent">{firm.name[0]}</span>
                        }
                      </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-4">
                      {firm.allows_scalping && <span className="px-2 py-0.5 bg-accent/10 text-accent text-[10px] rounded-md font-medium">✓ Scalping</span>}
                      {firm.allows_news_trading && <span className="px-2 py-0.5 bg-accent/10 text-accent text-[10px] rounded-md font-medium">✓ News Trading</span>}
                      {firm.allows_ea && <span className="px-2 py-0.5 bg-accent/10 text-accent text-[10px] rounded-md font-medium">✓ EAs/Bots</span>}
                    ...
                    <div className="flex items-center gap-2 mt-4">
                      <a
                        ...
                        className="flex-1 py-2.5 bg-accent-hover hover:bg-accent-hover text-white text-sm font-semibold rounded-xl flex items-center justify-center gap-1.5 transition-colors"
                      >
                      ...
                      <button
                        ...
                        className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center gap-1.5 border ${
                          isInCompare
                            ...
                            : 'bg-dark-700 text-text-secondary border-border hover:border-blue-500/40 hover:text-blue-400'
                        }`}
                      >
                      ...
                      <Link
                        href={`/${locale}/compare`}
                        className="px-4 py-2.5 rounded-xl text-sm font-medium bg-dark-700 text-text-secondary border border-border hover:border-border-hover hover:text-white transition-all"
                      >
                        Details
                      </Link>
                    </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="flex-1 py-3 bg-dark-700 hover:bg-dark-600 text-text-secondary text-sm font-medium rounded-xl flex items-center justify-center gap-2 transition-colors"
            >
      ...
      <div className="w-full max-w-lg">
        ...
        >
          ...
          <div className="mb-8">
            ...
            <p className="text-text-muted text-sm">{currentQuestion.subtitle}</p>
          </div>
              ...
              return (
                ...
                >
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${
                    isSelected ? colors.bg : 'bg-dark-700 group-hover:bg-dark-600'
                  }`}>
                    <Icon className={`w-5 h-5 transition-colors ${isSelected ? colors.icon : 'text-text-muted'}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`font-semibold text-sm transition-colors ${isSelected ? colors.text : 'text-white'}`}>
                      {opt.label}
                    </p>
                    <p className="text-text-muted text-xs mt-0.5">{opt.sub}</p>
                  </div>
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handleBack}
              className="p-3 rounded-xl bg-dark-700 hover:bg-dark-600 text-text-secondary hover:text-white transition-all"
            ...
            <button
              onClick={handleNext}
              disabled={!selected}
              className="flex-1 py-3.5 bg-accent-hover hover:brightness-110 disabled:bg-dark-700 disabled:text-text-muted disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 disabled:shadow-none"
            >
