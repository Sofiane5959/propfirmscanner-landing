...
// =====================================================
// TRUST/VERIFIED BADGE WITH TOOLTIP
// =====================================================
const TrustBadge = ({ status }: { status: string }) => {
  const config = {
    verified: { 
      ...
      label: 'Verified', 
      icon: BadgeCheck,
      tooltip: 'This prop firm has been scanned and verified by PropFirmScanner. We confirmed: active payouts, real business presence, and positive trader feedback.'
    },
    banned: { 
      bg: 'bg-red-500/20', 
      text: 'text-red-400', 
      ...
      label: 'Avoid', 
      icon: X,
      tooltip: 'Warning: This firm has reported issues with payouts or business practices. We recommend avoiding.'
    },
    under_review: { 
      bg: 'bg-yellow-500/20', 
      ...
      label: 'Under Review', 
      icon: AlertTriangle,
      tooltip: 'This firm is currently being reviewed by our team. Proceed with caution.'
    },
    new: { 
      bg: 'bg-blue-500/20', 
      text: 'text-blue-400', 
      ...
      label: 'New', 
      icon: Sparkles,
      tooltip: 'This firm was recently added to our database. Verification in progress.'
    },
  }
  ...
  return (
    <Tooltip content={tooltip}>
      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${bg} ${text} border ${border} cursor-help transition-all hover:opacity-80`}>
  ...
  return (
    <div className="absolute top-2 right-2 z-10">
      <div className="bg-gradient-to-r from-red-500 to-orange-500 text-white px-2 py-1 rounded text-xs font-bold shadow-lg">
        {percent}% OFF
        {code && (
          <button onClick={handleCopy} className="flex items-center gap-1 text-[10px] font-normal opacity-90 hover:opacity-100 mt-0.5">
            {copied ? <><CheckCircle2 className="w-2.5 h-2.5" />Copied!</> : <><Copy className="w-2.5 h-2.5" />{code}</>}
          </button>
        )}
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:w-[500px] max-h-[80vh] bg-gray-900 border border-gray-700 rounded-t-2xl sm:rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
          <h3 className="text-lg font-bold text-white flex items-center gap-2"><SlidersHorizontal className="w-5 h-5 text-emerald-400" />Advanced Filters</h3>
        ...
        <div className="p-4 space-y-6 max-h-[60vh] overflow-y-auto">
          <div>
            ...
            <select value={filters.platform || ''} onChange={(e) => setFilters({ ...filters, platform: e.target.value || null })} className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-emerald-500">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {CHALLENGE_TYPES.map(type => (
                <button key={type} onClick={() => setFilters({ ...filters, challengeType: type === 'All' ? null : type })} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${(type === 'All' && !filters.challengeType) || filters.challengeType === type ? 'bg-emerald-500 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{type}</button>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button key={key} onClick={() => setFilters({ ...filters, [key]: !filters[key] })} className={`px-3 py-2 rounded-lg text-xs font-medium transition-all flex items-center gap-2 ${filters[key] ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-gray-800 text-gray-400 border border-transparent hover:bg-gray-700'}`}>
                  {filters[key] ? <Check className="w-3.5 h-3.5" /> : <div className="w-3.5 h-3.5" />}{label}
                </button>
              ))}
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Max Price: {filters.maxPrice ? `$${filters.maxPrice}` : 'Any'}</label>
            <input type="range" min="0" max="1000" step="50" value={filters.maxPrice || 1000} onChange={(e) => setFilters({ ...filters, maxPrice: parseInt(e.target.value) === 1000 ? null : parseInt(e.target.value) })} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
            <div className="flex justify-between text-xs text-gray-500 mt-1"><span>$0</span><span>$500</span><span>Any</span></div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Min Profit Split</label>
            <div className="flex flex-wrap gap-2">
              {[null, 70, 80, 90, 95].map(split => (
                <button key={split || 'any'} onClick={() => setFilters({ ...filters, minProfitSplit: split })} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${filters.minProfitSplit === split ? 'bg-emerald-500 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{split ? `${split}%+` : 'Any'}</button>
              ))}
            </div>
          </div>
        </div>
        <div className="p-4 border-t border-gray-700 flex gap-3">
          <button onClick={() => setFilters({})} className="flex-1 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"><RotateCcw className="w-4 h-4" /> Reset All</button>
          <button onClick={onClose} className="flex-1 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium rounded-lg transition-colors">Apply Filters</button>
        </div>
      </div>
    </div>
  )
...
export default function ComparePageClient({ firms }: ComparePageClientProps) {
  ...
  const platforms = useMemo(() => Array.from(new Set(processedFirms.flatMap(f => f.platforms || []))).sort(), [processedFirms])
  const challengeTypes = useMemo(() => Array.from(new Set(processedFirms.flatMap(f => f.challenge_types || []))).sort(), [processedFirms])
  ...
  return (
    <div className="min-h-screen bg-gray-900">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <Tooltip content="All prop firms listed here have been manually scanned and verified by our team. We check: business legitimacy, payout history, and trader reviews.">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/30 rounded-full cursor-help">
            ...
            <div className="flex items-center gap-2">
              <div className="flex border border-gray-700 rounded-lg overflow-hidden">
                {[{ m: 'grid' as const, i: Grid3X3, label: 'Grid view' }, { m: 'list' as const, i: List, label: 'List view' }, { m: 'table' as const, i: BarChart3, label: 'Table view' }].map(({ m, i: Icon, label }) => (
                  <button key={m} onClick={() => setViewMode(m)} className={`p-1.5 ${viewMode === m ? 'bg-emerald-500 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`} aria-label={label}><Icon className="w-4 h-4" /></button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <div className="relative flex-1 min-w-[200px] max-w-[300px]">
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search firms..." className="w-full pl-8 pr-3 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm placeholder:text-gray-500 focus:outline-none focus:border-emerald-500" aria-label="Search prop firms" />
            </div>
            
            <div className="flex items-center gap-1">
              {MARKET_TYPES.slice(0, 5).map(market => (
                <button key={market} onClick={() => setSelectedMarket(market === 'All' ? null : market)} className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all ${(market === 'All' && !selectedMarket) || selectedMarket === market ? 'bg-emerald-500 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>{market}</button>
              ))}
            </div>
            
            {stats.withDiscounts > 0 && (
              <button onClick={() => setFilters({ ...filters, hasDiscount: !filters.hasDiscount })} className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center gap-1 ${filters.hasDiscount ? 'bg-orange-500/20 text-orange-400 ring-1 ring-orange-500/50' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                <Tag className="w-3 h-3" /> Deals
              </button>
            )}
            
            <Tooltip content="Show only verified and trusted prop firms">
              <button onClick={() => setShowOnlyVerified(!showOnlyVerified)} className={`px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center gap-1 ${showOnlyVerified ? 'bg-emerald-500/20 text-emerald-400 ring-1 ring-emerald-500/50' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`} aria-label="Toggle verified only filter">
                <BadgeCheck className="w-3 h-3" /> Verified
              </button>
            </Tooltip>
            
            <select value={filters.minRating || ''} onChange={(e) => setFilters({ ...filters, minRating: e.target.value ? parseFloat(e.target.value) : null })} className="px-2.5 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-gray-400 text-xs focus:outline-none focus:border-emerald-500" aria-label="Filter by minimum rating">
              {RATING_OPTIONS.map(opt => (<option key={opt.label} value={opt.value || ''}>{opt.label}</option>))}
            </select>
            
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value as typeof sortBy)} className="px-2.5 py-1.5 bg-gray-800 border border-gray-700 rounded-lg text-gray-400 text-xs focus:outline-none focus:border-emerald-500" aria-label="Sort results">
              {SORT_OPTIONS.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
            </select>
            
            <button onClick={() => setShowAdvancedFilters(true)} className="px-2.5 py-1.5 rounded-lg text-xs font-medium bg-gray-800 text-gray-400 hover:bg-gray-700 flex items-center gap-1" aria-label="Show more filters">
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap items-center gap-2 pb-2">
              {activeFilters.map(chip => (<FilterChip key={chip.key} label={chip.label} onRemove={chip.onRemove} />))}
              {activeFilters.length > 0 && (
                <button onClick={resetAllFilters} className="text-xs text-gray-500 hover:text-red-400 flex items-center gap-1"><RotateCcw className="w-3 h-3" /> Reset</button>
              )}
            </div>
          )}
        </div>
            ...
            <>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-6">
                  <button onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="p-2 rounded-lg bg-gray-800 text-gray-400 hover:bg-gray-700 disabled:opacity-50" aria-label="Previous page"><ChevronLeft className="w-4 h-4" /></button>
                  {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                    const p = totalPages <= 5 ? i + 1 : currentPage <= 3 ? i + 1 : currentPage >= totalPages - 2 ? totalPages - 4 + i : currentPage - 2 + i
                    return <button key={p} onClick={() => setCurrentPage(p)} className={`w-8 h-8 rounded-lg text-sm font-medium ${currentPage === p ? 'bg-emerald-500 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`} aria-label={`Page ${p}`}>{p}</button>
                  })}
                  <button onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="p-2 rounded-lg bg-gray-800 text-gray-400 hover:bg-gray-700 disabled:opacity-50" aria-label="Next page"><ChevronRight className="w-4 h-4" /></button>
                </div>
              )}
            </>
          )}
      ...
      <AdvancedFiltersPanel isOpen={showAdvancedFilters} onClose={() => setShowAdvancedFilters(false)} filters={filters} setFilters={setFilters} platforms={platforms} challengeTypes={challengeTypes} />
    </div>
  )
}
